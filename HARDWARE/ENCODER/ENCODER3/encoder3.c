...
static void encoder_rotation_scan(void) {
    ...
    if(encoder_counter % 4 == 0 && encoder_counter != 0) {
        if(encoder_counter == 4) {
            if(GET_ENCODER_KEY_STATE == ENCODER3_KEY_DOWN) {
                // 按住左旋回调
...
static void encoder_key_scan(void) {
    if(GET_ENCODER_KEY_STATE == ENCODER3_KEY_DOWN  && encoder3_key_state == 1) {
        encoder3_key_state = ENCODER3_KEY_DOWN;
        // 编码器按下回调
        encoder3_key_down_callback();
    }else if(GET_ENCODER_KEY_STATE == ENCODER3_KEY_UP && encoder3_key_state == 0) {
